...
// REPLACE THIS WITH YOUR CODE
const formalGreeting = (arr) => {
  const helloArr = arr.map((el) => {
    return `Hello, ${el}`;
  });
  return helloArr;
};
...
orders.forEach((el) => {
  ...
  console.log(el.tax + 1);
  console.log("-------");
  orderTotals.push(el.price * (el.tax + 1));
});
